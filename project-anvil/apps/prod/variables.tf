...
variable "monitoring_provider" {
  description = "The dashboarding and metrics provider to use ('aws_cloudwatch')."
  type        = string
  default     = "aws_cloudwatch"
}

variable "logging_provider" {
  description = "The log archiving and analytics provider to use ('aws_s3_firehose', 'aws_opensearch')."
  type        = string
  default     = "aws_s3_firehose"
}

variable "database_provider" {
  description = "The database provider to use for the data tier ('aws_rds')."
  type        = string
  default     = "aws_rds"
}

variable "alerting_provider" {
  description = "The destination service for all alerts ('pagerduty')."
  type        = string
  default     = "pagerduty"
}

variable "apm_provider" {
  description = "The Application Performance Monitoring (APM) provider to use ('aws_xray')."
  type        = string
  default     = "aws_xray"
}

variable "rum_provider" {
  description = "The Real User Monitoring (RUM) provider to use ('aws_rum')."
  type        = string
  default     = "aws_rum"
}
