...
# backend.tf
#
# Backend configuration for storing the Terraform state for the platform layer.
# The actual bucket, key, and DynamoDB table values are provided dynamically
# via -backend-config CLI flags from the CI/CD pipeline during 'terraform init'.
#
...
terraform {
  backend "s3" {
    ...
    #
    # Your CI/CD workflow (e.g., 2-deploy-platform.yml) passes these dynamically:
    # terraform init \
    #   -backend-config="bucket=${PLATFORM_BUCKET_NAME}" \
    #   -backend-config="key=${PLATFORM_KEY}" \
    #   -backend-config="region=${AWS_REGION}" \
    #   -backend-config="dynamodb_table=${PLATFORM_DDB_TABLE_NAME}" \
