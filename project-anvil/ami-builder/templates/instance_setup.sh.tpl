...
# templates/instance_setup.sh.tpl
#
# This user data script runs on EC2 instance boot-up. It dynamically configures
# the instance based on its identified role (web or app server). This includes
# internal TLS certificate generation, X-Ray daemon setup, CloudWatch agent
# configuration, Nginx/PHP-FPM setup, and WordPress database configuration.
#
# This file is a Terraform template (hence the .tpl extension) and its variables
# are injected by the calling Terraform module during the instance launch.
#
...
# These values are passed by Terraform's templatefile() function from the
# calling module (web_tier or app_tier).
# ------------------------------------------------------------------------------
PCA_ARN="${certificate_authority_arn}"
AWS_REGION="${aws_region}"
CMS_NAME="${cms_name}"
CMS_VERSION="${cms_version}"
ENV="${env}"                          # Explicit environment name (e.g., 'dev')
DOMAIN_NAME="${domain_name}"
APP_TIER_DNS="${app_tier_dns}"        # Internal DNS name of App Tier's Load Balancer
DB_ENDPOINT="${db_endpoint}"          # Database endpoint
DB_PORT="${db_port}"                  # Database port
DB_NAME="${db_name}"                  # Database name
DB_USERNAME="${db_username}"          # Database username
XRAY_ENABLED="${xray_enabled}"        # Boolean: 'true' if X-Ray is enabled
DB_PASSWORD_SECRET_ARN="${db_password_secret_arn}" # ARN of DB master password secret
WP_SALTS_SECRET_ARN="${wp_salts_secret_arn}"       # ARN of WordPress salts secret
RUM_APP_MONITOR_SCRIPT="${rum_app_monitor_script}" # RUM script string

# ------------------------------------------------------------------------------
# Constants (Paths and Service Names)
# ------------------------------------------------------------------------------
CERT_DIR="/etc/ssl/acme_internal"
APP_DIR="/var/www/wordpress"         # Standard WordPress app location
NGINX_CONF_PATH="/etc/nginx/sites-available/default"
PHP_FPM_WWW_CONF="/etc/php/8.3/fpm/pool.d/www.conf" # Assumed PHP version (adjust if different)
...
# Function to get instance ID from EC2 metadata
get_instance_id() {
...
setup_xray_daemon() {
  if [[ "$XRAY_ENABLED" == "true" ]]; then
    ...
    wget https://s3.amazonaws.com/aws-xray-assets."$AWS_REGION"/xray-daemon/aws-xray-daemon-3.x.deb -O /tmp/aws-xray-daemon.deb
...
install_initial_tls() {
  ...
  local MAX_WAIT_ATTEMPTS=60 # 5 minutes total (60 attempts * 5 seconds sleep)
  local ATTEMPT=0
...
configure_nginx() {
  ...
  # Add conditional check for APP_TIER_DNS in case it's not applicable (e.g., in a single-tier setup or for app tier itself).
  if [ -n "$APP_TIER_DNS" ]; then
    ...
    log_message "APP_TIER_DNS is empty, skipping Nginx app tier configuration."
  fi
...
configure_wordpress() {
  ...
  if [ -z "$DB_PASSWORD_SECRET_ARN" ]; then
    log_message "Warning: DB_PASSWORD_SECRET_ARN is empty. Skipping DB password fetch. This may cause issues." >&2
    DB_PASSWORD_ACTUAL="YOUR_DB_PASSWORD_HERE" # Fallback, will likely cause connection failure
  else
  ...
  if [ -z "$WP_SALTS_SECRET_ARN" ]; then
    log_message "Warning: WP_SALTS_SECRET_ARN is empty. Skipping WordPress salts fetch. This may cause issues." >&2
    WP_SALTS_JSON="{}" # Fallback to empty JSON
  else
...
inject_rum_script() {
  if [ -n "$RUM_APP_MONITOR_SCRIPT" ]; then # Only inject if script is provided
    log_message "Injecting RUM app monitor script..."
    # For WordPress, you'd usually append this to wp-config.php or use a plugin.
    # This is an example; exact injection depends on WordPress setup.
    echo "$RUM_APP_MONITOR_SCRIPT" | sudo tee /var/www/html/rum_script.js > /dev/null # Example: writes to a JS file
    log_message "RUM app monitor script injected to /var/www/html/rum_script.js."
  else
    log_message "RUM app monitor script not provided. Skipping injection."
  fi
}

# ------------------------------------------------------------------------------
# Main Execution Flow (Instance Role Identification)
# This section identifies the instance's role (Web or App Server) based on
# active services and then applies role-specific configurations.
# ------------------------------------------------------------------------------
...
if systemctl is-active --quiet nginx; then
  ...
  setup_xray_daemon
  install_initial_tls "web.${ENV}" # Use explicit ENV
  configure_nginx
  ...
  setup_xray_daemon
  install_initial_tls "app.${ENV}" # Use explicit ENV
  configure_wordpress # WordPress config only goes on the App Tier
  inject_rum_script # RUM script also used on App Tier if it serves web content directly
