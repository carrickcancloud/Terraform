...
#
# Packer template for building the Golden AMI for the Application Server tier.
# It provisions an EC2 instance, installs PHP-FPM, WordPress core files,
# caching plugins, and the CloudWatch Agent. It also includes Trivy for
...
variable "tier" {
  ...
  default     = "app" # Default for this template
}
...
build {
  ...
  # Step 1: Upload the app tier's package (WordPress core files).
  ...
  provisioner "file" {
    source      = "../../dist/app_package.tar.gz" # Path relative to ami-builder/packer/
  ...
  # This path is relative to the root of the Packer execution directory (ami-builder/packer).
  provisioner "file" {
    source      = "../scripts/install_app.sh" # Path relative to ami-builder/packer/
    destination = "/tmp/install_app.sh"
  }

  # Step 3: Execute the installation script.
  # This script installs PHP-FPM, unpacks application code, and installs CloudWatch Agent.
  provisioner "shell" {
    environment_vars = [
    ...
    inline = [
      "echo '--- [Packer Debug] /tmp directory contents before install_app.sh ---'",
      "ls -l /tmp",
