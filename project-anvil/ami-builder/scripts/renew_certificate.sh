...
#
# This script is deployed as part of the AMI and runs periodically (e.g., via cron)
# on application instances to renew internal TLS certificates issued by AWS Private CA.
# It re-uses the private key for renewal. This ensures instances always have
# valid internal TLS certificates for mTLS or service-to-service encryption.
#
...
# with actual values during instance launch via user_data.
...
# Checks for existing cert.pem and extracts its Common Name.
get_common_name_from_cert() {
  if [ -f "$CERT_DIR/cert.pem" ]; then
    sudo openssl x509 -in "$CERT_DIR/cert.pem" -subject -noout | sed -n '/CN=/s/.*CN=\([^/]*\).*/\1/p'
  else
    echo "" # Return empty if no cert found
  fi
}

# Function to request and install new certificate from ACM PCA
# Generates a CSR, requests a new certificate from the specified PCA, waits
# for issuance, and installs the new certificate and chain.
request_and_install_cert() {
  local common_name="$1"
  log_message "Requesting new certificate for CN: $common_name from PCA: $PCA_ARN"

  # Ensure private key exists before generating CSR
  ...
  # Final check if certificate was received
  if ! echo "$CERT_CHAIN_JSON" | jq -e '.Certificate' > /dev/null; then
...
if [ -z "$COMMON_NAME" ]; then
  ...
  exit 0 # Exit successfully, as there's nothing to renew yet
fi

# Get certificate expiration date (Unix timestamp)
...
if [ "$DAYS_TO_EXPIRY" -le "$RENEWAL_WINDOW_DAYS" ]; then
  ...
  if request_and_install_cert "$COMMON_NAME"; then
    ...
    # This requires 'SERVICE_TO_RELOAD' to be correctly set (e.g., 'nginx', 'php8.3-fpm').
